...
def extract(book, warnings):
    ...
    def cut_candidates(sname, require_trench_label=False):
        out = []  # (lvl, cells, "<sheet> row N")
        for rn, _h, lvl, cells in rows_of(book, sname):
            lab = label_of(cells)
            ...
            if require_trench_label and (
                ...
                continue
            out.append((lvl, cells, "%s row %d" % (sname, rn)))
        return out
    ...
    for sname in candidate_sheets:
        cands += cut_candidates(sname)
    if not cands:
        # Fallback: trench-labelled cut rows on any other sheet
        for sname in book:
            if sname in candidate_sheets:
                continue
            cands += cut_candidates(sname, require_trench_label=True)

    def lo_level(c):
        return min((lvl for lvl, _c, _s in c), default=None)

    lo = lo_level(cands)
    if lo is not None:
        chosen = [c for l, c, _s in cands if l == lo]
        data["trench_cut"] = {
        ...
        data["trench_cut_sources"] = [s for l, _c, s in cands if l == lo]

        # Sanity: trench cut can never exceed the whole site's cut/export;
        # if it does, the export is almost certainly double counting.
        tc_check = data["trench_cut"]
        tsc_check = data.get("total_site_cut")
        data["trench_cut_anomaly"] = False
        if tsc_check is not None:
            for col, key in ((2, "exported"), (3, "reused"), (4, "cut")):
                limit = num(tsc_check, col)
                if limit is None:
                    limit = 0.0
                if tc_check[key] > limit + 1e-6:
                    data["trench_cut_anomaly"] = True
    else:
        data["trench_cut"] = None
        data["trench_cut_sources"] = []
        data["trench_cut_anomaly"] = False
        warnings.append(
...
WARN = 8                       # red bold - sanity warnings
...
def build_summary_rows(data):
    ...
    # Sanity warning (e.g. export double-counts trench volumes) ---------------
    if data.get("trench_cut_anomaly"):
        srcs = data.get("trench_cut_sources") or []
        note = ("!! CHECK: TRENCH CUT is bigger than TOTAL SITE CUT, so SITE "
                "CUT ONLY came out NEGATIVE. This usually means the export "
                "double-counts trench volumes - verify against Mudshark.")
        if srcs:
            shown = srcs if len(srcs) <= 14 else srcs[:14] + ["..."]
            note += "  Rows summed: " + ", ".join(shown)
        add_row({1: (note, WARN)})
    else:
        add_row()
    add_row()
...
STYLES = ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
          ...
          '<font><b/><sz val="8"/><color rgb="FFFF0000"/>'
          '<name val="Microsoft Sans Serif"/></font>'
          '</fonts>'
          '<fills count="4">'
...
def main(argv):
    ...
    if data.get("trench_cut_anomaly"):
        warnings.append(
            "TRENCH CUT is bigger than TOTAL SITE CUT (SITE CUT ONLY is\n"
            "         negative). A red CHECK note was written into the Summary.\n"
            "         Rows summed for TRENCH CUT: %s"
            % ", ".join(data.get("trench_cut_sources") or ["?"]))
